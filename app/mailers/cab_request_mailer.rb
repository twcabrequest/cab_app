...
# send a signup email to the user, pass in the user object that contains the user's email address
  def send_email(cab_request,pick_up_date,pick_up_time,requester,admin_emails)
  ...
  def send_vendor_email(cab_request,pick_up_date,pick_up_time,requester,admin_emails,vendor_email,vendor_id)
    mail(reply_to:[requester,admin_emails],  cc: [vendor_email] , subject: "[Cab Request]", content_type: "text/html", body:

        "<form action='http://localhost:3000/vendor_response/"+ cab_request.id.to_s + "/submit_response'>" +
